...
and backend part made using [Google Firebase](https://firebase.google.com/). As a build tool I
...
- features - reused implementations of entire product features, i.e. actions that bring business value to the user:
  `AddFriend`, `AddMessage`, `AddPost`, etc.
...
Also, I configured [aliasing paths](https://dev.to/tilly/aliasing-in-vite-w-typescript-1lfo) in the project for all
layers.
For linting & formatting I use [Biome](https://biomejs.dev).
For now not all Components were divided
into [presentational and container](https://medium.com/@dan_abramov/smart-and-dumb-components-7ca2f9a7c7d0).

## State management

I decided to use [RTK](https://redux-toolkit.js.org/) with RTK Query for state management because I wanted to have a
centralized state
